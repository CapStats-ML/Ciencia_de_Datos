...
summary(data)
write_csv(cbind(Y, data), "data3.csv")
...
data <- data[, -c(6, 7, 8)]
data[, c(6:17)] <- lapply(data[, c(6:17)], factor)
...
plot(FA, choix = "var", axes = c(1,2), lab.ind = FALSE, graph.type = "ggplot")
plot(FA, choix = "ind", axes = c(1,2), lab.ind = FALSE, graph.type = "ggplot")
plot(FA, choix = "quanti", axes = c(1,2), lab.ind = FALSE, graph.type = "ggplot")
plot(FA, choix = "quali", axes = c(1,2), lab.ind = FALSE, graph.type = "ggplot")
...
Dat <- data.frame(FA$ind$coord)
summary(Dat)
write_csv(cbind(Y, Dat), "FAMD.csv")
