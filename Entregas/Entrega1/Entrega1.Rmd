...
```{r plot-TipoAdmision1, message=FALSE, warning=FALSE}
T_Prop_Admision %>% 
  ggplot(aes(x = reorder(Variable, Proporcion), y = Proporcion)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
...
# Calcula proporciones para la variable transformada
T_Prop_Admision_Trans <- data %>%
  count(Admision) %>%
  mutate(Proporcion = n / sum(n))
```

```{r plot-TipoAdmision2, message=FALSE, warning=FALSE}
...
```{r plot-Especialidad1, message=FALSE, warning=FALSE}
...
# Calcula proporciones para la variable transformada
T_Prop_Especialidad_Trans <- data %>%
  count(Especialidad) %>%
  mutate(Proporcion = n / sum(n))

```

```{r plot-Especialidad2, message=FALSE, warning=FALSE}
...
```{r plot-Diag11, message=FALSE, warning=FALSE}
T_Prop_diag1 %>%
  ggplot(aes(x = reorder(Variable, Proporcion), y = Proporcion)) +
  geom_bar(stat = "identity", fill = "steelblue") +
...
# Calcular proporciones para la variable transform
...
```{r plot-Diag12, message=FALSE, warning=FALSE}
...
```{r plot-Diag21, message=FALSE, warning=FALSE}
...
```{r plot-Diag22, message=FALSE, warning=FALSE}
...
```{r plot-Diag31, message=FALSE, warning=FALSE}
T_Prop_diag3 %>%
  ggplot(aes(x = reorder(Variable, Proporcion), y = Proporcion)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_hline(yintercept = 0.1, linetype = "dashed", color = "red", size = 0.7) + # Línea horizontal
...
```{r plot-Diag32, message=FALSE, warning=FALSE}
...
corrplot(correlation_matrix, 
         ...
         col = colorRampPalette(c('#F5A831', '#D14B3D', '#1D2F57'))(200),  # Definir la paleta de colores
